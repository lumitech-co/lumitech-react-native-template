...
export const testQueryWithFetchQueryFnAuthService = async ({
  params,
  signal,
}: QueryFnParams) => {
  const response = await AuthService.testQueryWithFetch(params, { signal });

  return response;
};

const getQueryKey = (params: Test) =>
  queryKeys.testQueryWithFetchAuthService(params);

export const testQueryWithFetchQueryAuthService = <
...
>({
  ...
  >({
    ...
    queryFn: ({ signal }) =>
      testQueryWithFetchQueryFnAuthService({ params, signal }),
    ...fetchOptions,
  });
};

export const useTestQueryWithFetchQueryAuthService = <
  TData = CreateAccountResponse[],
>({
  options,
  ...params
...
export const invalidateTestQueryWithFetchQueryAuthService = (
  params: Test,
...
export const useTestQueryWithFetchAuthServiceObservable = <
  TData = CreateAccountResponse[],
  TSelected = TData,
>({
  params$,
  ...
  return useObservable(
    ...
    >({
      queryClient,
      queryFn: ({ signal }) =>
        testQueryWithFetchQueryFnAuthService({ params: params$, signal }),
